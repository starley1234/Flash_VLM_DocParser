...
`http://localhost:8000/` — минималистичное приложение для работы с очередью:

- **Иерархия «документ → страницы»** слева: страницы свёрнуты по умолчанию; у
  обрабатываемого документа видно текущую страницу (пульсирующая точка), у
  страниц — статус (готово / обработка / ожидание / ошибка).
- **Клик по документу** — текущий (потоково обновляемый) результат; **клик по
  странице** — её Markdown и превью уменьшенного изображения с открытием в новом
  окне (для контроля ресайза).
- **Настройки в аккордеоне**: системный промпт (предзаполнен текущим), ширина
  изображения и модель — применяются к новым документам.
- Документы можно добавлять независимо и смотреть результаты параллельно.

Изображения страниц сохраняются на диск рядом с итоговым `.md` в каталог
...
| `GET` | `/settings` | Настройки для UI (промпт, ширина, модель) |
| `GET` | `/jobs` | Список задач (+ компактные статусы страниц) |
| `GET` | `/jobs/{id}` | Статус, прогресс и страницы задачи |
